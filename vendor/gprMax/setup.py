...
if sys.platform == "win32":
    compile_args = [
        ...
        "/openmp",
        "/w",
    ]  # No static linking as no static version of OpenMP library; /w disables warnings
    linker_args = []
    extra_objects = []
    libraries = []
...
#          - with gcc -fopenmp option implies -pthread
    ...
    cpuID = (
        subprocess.check_output(
            "sysctl -n machdep.cpu.brand_string", shell=True, stderr=subprocess.STDOUT
        )
        .decode("utf-8")
        .strip()
    )
    ...
    if "Apple" in cpuID:
        ...
        if gccpath:
            ...
            raise (
                "Cannot find gcc 4-10 in /opt/homebrew/bin. gprMax requires gcc to be installed - easily done through the Homebrew package manager (http://brew.sh). Note: gcc with OpenMP support is required."
            )
    else:
        ...
        if gccpath:
            ...
            os.environ["CC"] = gccpath[-1].split(os.sep)[-1]
            rpath = (
                "/usr/local/opt/gcc/lib/gcc/" + gccpath[-1].split(os.sep)[-1][-1] + "/"
            )
        else:
            raise (
                "Cannot find gcc 4-10 in /usr/local/bin. gprMax requires gcc to be installed - easily done through the Homebrew package manager (http://brew.sh). Note: gcc with OpenMP support is required."
            )
    compile_args = [
        "-O3",
        "-w",
